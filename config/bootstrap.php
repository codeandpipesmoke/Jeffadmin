...
Configure::write('Jeffadmin', ['version' => '0.0.1']);

try {
    Configure::load('sidebarmenu', 'default');
} catch (\Exception $e) {
	try {
		Configure::load('Jeffadmin.sidebarmenu', 'default');
	} catch (\Exception $e) {
		exit($e->getMessage() . "\n");
	}
}
...
try {
	...
	try {
		Configure::load('Jeffadmin.jeffadmin', 'default');
	} catch (\Exception $e) {
		exit($e->getMessage() . "\n");
	}
}
...
try {
    Configure::load('Jeffadmin.templates', 'default');
} catch (\Exception $e) {
    exit($e->getMessage() . "\n");
}
